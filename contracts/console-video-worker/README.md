...
- `FinishRecording`: 指定takeを停止し、MP4をflushしてcloseし、一時pathから`relative_path`へatomicにrenameする。完成ファイルをupload queueへ永続化してから`RECORDING_STATE_FINISHED`を通知する。
- `Shutdown`: 録画していないworkerを正常終了させる。workerはterminalな`CommandResult`を送信してstreamを閉じ、exit status 0で終了する。録画中は`REJECTED`を返す。uploadingなVideoFileがある場合の通常削除はconsole-serverが拒否し、強制削除ではVideoFileをerroredにしてからworkerを終了させる。
...
- workerは録画状態の変化を`recording_status_changed`で通知する。`FINISHED`はファイルのflush、close、renameおよびupload queueへの永続化が完了したことを表す。
- workerは録画ファイルのhash計算とobject storageへのuploadを行い、terminalな結果を`ReportUpload`でconsole-serverへ通知する。console-serverがVideoFileとFinishedTakeの更新をcommitしてacknowledgementを返すまで、workerは同じreportを再送する。
- `ReportUpload`の`COMPLETED`では`object_key`、32 byteのSHA-256および0以上の`size`を必須とし、`ERRORED`では空でない`error`を必須とする。`(take_id, camera_identity_id)`を冪等性keyとする。
...
- gRPC streamの切断、deadline超過またはconsole-serverの停止だけを理由に、入力受付、preview、録画、ファイル確定またはupload処理を停止してはならない。workerは指数backoff付きで再接続する。
