...
- video gateway: RIST Main Profileの接続を待ち受け、ARQによるpacket loss回復と並べ替えを行う。復旧後packetのsequence numberとtimestampを維持したRTP/MP2Tを、payloadを解釈することなく対応するvideo workerへ中継する。また、復旧後のsequence列とlibrist統計を観測し、OpenTelemetry Metricsとしてconsole serverへ送信する。
- video worker: CameraConnectionごとに最大1つのPodとして配置される。そのPod内のvideo worker containerは、対応するcameraのRTP payloadを解釈し、映像形式とframe rateを検証する。要求を満たす映像をshared volumeへ録画し、リアルタイムプレビュー用の映像をLiveKitへ中継する。
