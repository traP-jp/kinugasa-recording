...
- video gateway: RIST Main Profileの接続を待ち受け、cameraからH.264映像および音声を受信し、RTPへ再パケット化してvideo hubへ中継する。
- video hub: 必要に応じてsessionごとに最大1つ配置され、複数cameraのRTP映像を集約して録画し、リアルタイムプレビュー用の映像をLiveKitへ中継する。
