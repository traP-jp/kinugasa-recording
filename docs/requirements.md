...
- アクター: mocapチーム
- 事前条件: なし
- トリガー: kubernetesクラスタを新規に作成するラッパースクリプトを実行する。

#### 基本フロー
1. mocapチームがk3dを用いてkubernetesクラスタを作成する。環境変数`KINUGASA_RECORDING_SESSION_NAME`にsession名を指定する。

#### 例外フロー
1a. 指定されたsessionの名前が既に存在する、あるいは過去に使用されたことがある場合。
    1. 作成を行わず、k3dのコマンドライン上に警告を表示する。
1b. `KINUGASA_RECORDING_SESSION_NAME`に指定されたsession名が不正な場合。
    1. 作成を行わず、k3dのコマンドライン上に警告を表示する。
1c. `KINUGASA_RECORDING_SESSION_NAME`が指定されていない場合。
    1. 作成を行わず、k3dのコマンドライン上に警告を表示する。
...
- アクター: mocapチーム
- 事前条件: UC-006が完了している。
- トリガー: kubernetesクラスタを削除するラッパースクリプトを実行する。

#### 基本フロー
1. mocapチームがk3dを用いてkubernetesクラスタを削除する。
