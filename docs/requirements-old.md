...
### 1.3 成功条件
- [KPI-001] Let's Note CF-SRで動作する。この点については、現時点で説得力のある要件を提示できないため、後日検討する。
- [KPI-002] Webブラウザから操作できる。
- [KPI-003] 録画結果を複数の動画ファイルに分割し、書き込みが完了した動画ファイルから逐次的にオブジェクトストレージへアップロードできる。
...
3. camera関連のコンポーネントが、各cameraの映像を複数のMPEG-TSファイルに分割して録画し、書き込みが完了したファイルから逐次アップロードする。
...
### UC-005: オブジェクトストレージからの動画ファイル群の取得
- アクター: mocapチーム
- 事前条件: UC-002が完了している。
- トリガー: `kinugasa-recording`の対象外のフローであるため省略。

#### 基本フロー
1. session名、take名、camera名を指定してオブジェクトストレージから当該録画を構成する動画ファイル群を取得する。
...
#### FR-002-3: アップロード
`video uploader`は、録画中に書き込みが完了したMPEG-TSファイルを検出し、逐次的にオブジェクトストレージへアップロードする。
#### FR-002-4: 録画
`video recorder`は、各cameraの映像を複数のMPEG-TSファイルに分割し、shared volumeに保存する。
...
- video file: 1つのcameraからの映像入力を1つのtakeにおいて分割して録画した結果として生成されるMPEG-TSファイル。1つのtakeとcameraの組に対して複数生成され、オブジェクトストレージへアップロードされる最小単位となる。
