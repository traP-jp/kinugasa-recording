...
開発環境の初回構築、更新、Garageの確認、破棄は`docs/development.md`を参照する。

## 1. 前提

- x86_64 Linux、Nix flakes、Docker daemonを使用する。
- camera client、host PC、操作用browserは同じLANへ接続する。
- 実運用ではS3互換bucketを事前に作成する。bucket管理は本システムの対象外である。
  k3d開発環境に限り、開発補助としてGarageとbucketを自動作成する。
- 名前は英数字とハイフンだけを使用する。Session名はbucket内、Camera名とTake名はSession内で再利用できない。

## 2. S3とLiveKitの設定

k3d開発環境のGarage設定は`docs/development.md`を参照する。
