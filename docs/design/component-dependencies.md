...
flowchart LR
    ...
    subgraph cluster["Kubernetesクラスタ"]
        ...
        gateway["video gateway<br/>Rust / rist-rs / librist"]
        livekit["LiveKit"]
...
video gateway containerは、固定revisionの`rist-rs`を通じてlibristを使用するRust製binaryを実行する。libristのcallback内で復旧後packetからRTP headerを復元してvideo workerへ直接UDP送信し、application独自のpacket queueは設けない。video workerはRTP/MP2TからRTP transport headerを除去してMediaMTXのUDP MPEG-TS inputへ渡す。MPEG-TSのdemux、配信およびfMP4録画はMediaMTXが行い、video workerはMediaMTXのRTSP出力に対して映像形式とframe rateを検証する。
