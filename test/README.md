...
完了した各cameraのupload件数とS3上の全segmentを照合し、すべて取得して内容を検査する。
disk使用率が高い開発環境では、workloadごとのimageをPod起動直前にimportしてimage GCの影響を抑える。
Web unit testではRIST/SRT QRへ渡す値がAPIの接続URLと一致することを確認する。
